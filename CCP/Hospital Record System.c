/* hospital_records.c
   Hospital Patient Record Management System (Console)
   - Simple menu-driven program in C
   - Stores records persistently in a binary file ("patients.dat")
   - Supports add, display, search by ID, search by disease (partial),
     edit, delete, export to CSV.

   Compile:
     gcc -std=c11 -o hospital_records hospital_records.c
...
/* Utility: read a line from stdin, strip newline */
...
/* Generate next ID by scanning file for highest ID */
int next_id() {
    FILE *fp = fopen(DATAFILE, "rb");
    if (!fp) return 1; /* first id = 1 */
    Patient p;
    int max = 0;
...
/* Search by disease (partial, case-insensitive) */
...
/* Edit patient record (search by ID) */
void edit_patient() {
    char idbuf[32];
...
/* Delete patient by ID (rewrite file excluding that ID) */
void delete_patient() {
    char idbuf[32];
...
void export_csv() {
    ...
    int count = 0;
    while (fread(&p, sizeof(Patient), 1, fp) == 1) {
        /* escape double quotes by doubling them for CSV safety */
